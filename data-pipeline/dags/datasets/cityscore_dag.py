...
def validate_raw(**context) -> dict:
    ...
    if not result.is_valid:
        ...
        alert_validation_failure(dataset=DATASET, stage="raw", errors=errors, execution_date=execution_date, dag_id=DAG_ID, task_id="validate_raw")
...
def validate_processed(**context) -> dict:
    ...
    if not result.is_valid:
        errors = [str(e) for e in result.errors]
        alert_validation_failure(dataset=DATASET, stage="processed", errors=errors, execution_date=execution_date, dag_id=DAG_ID, task_id="validate_processed")
        if enforcer.config.validation.schema.strict_mode:
...
def pipeline_complete(**context) -> dict:
    ...
    execution_date = context["ds"]
    alert_pipeline_complete(dataset=DATASET, execution_date=execution_date, duration_seconds=0, stats={}, dag_id=DAG_ID)
