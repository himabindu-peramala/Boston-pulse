...
def detect_drift(**context) -> dict:
    """
    Detect data drift in processed CityScore data.
...
def check_fairness(**context) -> dict:
    """
    Check fairness metrics for the CityScore dataset.
    ...
    result = checker.evaluate_fairness(
        ...
        dataset=DATASET,
        outcome_column=None,
        dimensions=["score_name"],  # cityscore-specific: evaluate across service categories
    )
...
def mitigate_bias_task(**context) -> dict:
    ...
    mitigation_result = mitigator.mitigate(
        ...
        dimension="score_name",  # cityscore-specific dimension
        strategy=strategy,
    )
