...
class BiasDetector:
    ...
    def detect_geographic_bias(self):
        ...
        logger.info(f"Geographic bias analysis complete:")
    ...
    def detect_temporal_bias(self):
        ...
        logger.info(f"Temporal bias analysis complete:")
    ...
    def detect_severity_bias(self):
        ...
        if 'DISTRICT' in self.df.columns:
            ...
            overall_serious_pct = severity_stats['overall_distribution'].get('Serious', {}).get('percentage', 0)
            ...
            if biased_districts:
                logger.warning(f"Found {len(biased_districts)} districts with unusual severity patterns")
    ...
    def detect_coordinate_bias(self):
        ...
        if 'DISTRICT' in self.df.columns:
            coord_quality = self.df.groupby('DISTRICT')['valid_coords'].agg([
                ('total', 'count'),
                ('valid', 'sum'),
                ('invalid', lambda x: (x == 0).sum())
            ])
            
            coord_quality['invalid_percentage'] = (coord_quality['invalid'] / coord_quality['total']) * 100
            ...
            logger.info(f"Coordinate quality analysis:")
    ...
    def generate_bias_report(self, output_path: str = "data/processed/bias_report.txt"):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if geo_bias:
                f.write(f"Average crimes per district: {geo_bias.get('average_crimes_per_district', 0):.0f}\n\n")
                ...
                for item in geo_bias.get('district_distribution', [])[:5]:
                    f.write(f"  {item['district']}: {item['crime_count']} ({item['percentage']:.2f}%)\n")
            ...
            if temp_bias:
                f.write(f"Expected distribution per time period: {temp_bias.get('expected_percentage', 25):.1f}%\n\n")
                ...
                for item in temp_bias.get('time_distribution', []):
                    ...
                    f.write(f"  {item['time_period']}: {item['crime_count']} ({item['percentage']:.2f}%) [{status}]\n")
            ...
            if sev_bias:
                ...
                if sev_bias.get('biased_districts'):
                    ...
                    for item in sev_bias['biased_districts']:
                        f.write(f"  {item['district']}: {item['serious_crime_pct']:.1f}% serious crimes (deviation: {item['deviation']:.1f}%)\n")
    ...
    def visualize_bias(self, output_dir: str = "data/processed/visualizations"):
        ...
        if 'DISTRICT' in self.df.columns:
            ...
            plt.close()
            logger.info(f"Saved geographic bias visualization")
        ...
        if 'time_of_day' in self.df.columns:
            ...
            colors = ['#FF6B6B' if count > time_counts.mean() * 1.2 else '#4ECDC4' 
            ...
            plt.ylabel('Number of Crimes')
            plt.axhline(y=time_counts.mean(), color='red', linestyle='--', 
                       label=f'Average: {time_counts.mean():.0f}')
            plt.legend()
            plt.tight_layout()
            plt.savefig(f"{output_dir}/temporal_bias.png", dpi=300)
            plt.close()
            logger.info(f"Saved temporal bias visualization")
        ...
        if 'severity' in self.df.columns and 'DISTRICT' in self.df.columns:
            ...
            # Create crosstab
            severity_by_district = pd.crosstab(
                self.df['DISTRICT'], 
                self.df['severity'], 
                normalize='index'
            ) * 100
            ...
            sns.heatmap(severity_by_district, annot=True, fmt='.1f', cmap='RdYlGn_r', 
            ...
            plt.close()
            logger.info(f"Saved severity bias heatmap")
...
def main():
    ...
    print(f"📄 Full report: data/processed/bias_report.txt")
    print(f"📈 Visualizations: data/processed/visualizations/")
    print("=" * 80 + "\n")
