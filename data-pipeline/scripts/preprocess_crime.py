...
def preprocess_crime_data(
    ...
    try:
        ...
        logger.info(f"  New features added: year, month, day_of_week, hour, is_weekend, time_of_day, severity, district_crime_count")
...
def generate_statistics(file_path: str = "data/processed/crime_data_clean.csv"):
    ...
    try:
        ...
        stats = {
            ...
            'date_range': {
                'start': str(df['OCCURRED_ON_DATE'].min()) if 'OCCURRED_ON_DATE' in df.columns else None,
                'end': str(df['OCCURRED_ON_DATE'].max()) if 'OCCURRED_ON_DATE' in df.columns else None
            },
            'top_offenses': df['OFFENSE_DESCRIPTION'].value_counts().head(5).to_dict() if 'OFFENSE_DESCRIPTION' in df.columns else {},
