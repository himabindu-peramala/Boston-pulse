...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/stations_bias_detection.log"), logging.StreamHandler()],
)
...
class StationsBiasDetector:
    ...
    def detect_geographic_coverage_bias(self):
        ...
        logger.info("Geographic bias complete:")
        logger.info(f"  Overrepresented: {
                overrepresented['neighborhood'].tolist()}")
        logger.info(f"  Underrepresented: {
                underrepresented['neighborhood'].tolist()}")
    ...
    def detect_size_equity_bias(self):
        ...
        if "ZONE" in self.df.columns:
            ...
            for zone, row in zone_size.iterrows():
                ...
                if deviation_pct > 30:
                    biased_zones.append(
                        {"zone": zone, "avg_sqft": row["mean"], "deviation_pct": deviation_pct}
                    )

            size_stats["biased_zones"] = biased_zones
            if biased_zones:
                logger.warning(f"Found {
                        len(biased_zones)} zones with disproportionate station sizes")
    ...
    def detect_zone_distribution_bias(self):
        ...
        for zone, count in zone_counts.items():
            zone_stats.append(
                {"zone": zone, "station_count": count, "percentage": (count / total) * 100}
            )
    ...
    def detect_coordinate_quality_bias(self):
        ...
        if "NEIGHBORHOOD" in self.df.columns:
            ...
            if not problematic.empty:
                logger.warning(f"Neighborhoods with invalid coordinates: {
    ...
    def generate_bias_report(self, output_path: str = "data/processed/stations_bias_report.txt"):
        ...
        with open(output_path, "w", encoding="utf-8") as f:
            ...
            if geo:
                f.write(f"Avg stations per neighborhood: {
                        geo.get(
                            'avg_stations_per_neighborhood',
                            0):.2f}\n\n")
            ...
            if size:
                ...
                f.write(f"Mean sq footage: {
                        overall.get(
                            'mean_sqft',
                            0):.0f} ft²\n")
                f.write(f"Median sq footage: {
                        overall.get(
                            'median_sqft',
                            0):.0f} ft²\n")
                f.write(f"Range: {
            ...
            if zone:
                f.write(f"Expected distribution per zone: {
                        zone.get(
                            'expected_pct_per_zone',
                            33.3):.1f}%\n\n")
                for item in zone.get("zone_distribution", []):
                    ...
                    f.write(f"  {
                            item['zone']}: {
                            item['station_count']} stations ({
                            item['percentage']:.1f}%) [{status}]\n")
            f.write("\n")
    ...
    def visualize_bias(self, output_dir: str = "data/processed/stations_visualizations"):
        ...
        if "ZONE" in self.df.columns:
            ...
            plt.title(
                "Station Distribution: Inner / Mid / Outer Boston", fontsize=14, fontweight="bold"
            )
            plt.tight_layout()
...
def main():
    ...
    print("📈 Visualizations: data/processed/stations_visualizations/")
    print("=" * 80 + "\n")
