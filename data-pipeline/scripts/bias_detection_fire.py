...
class FireBiasDetector:
    ...
    def detect_severity_bias(self):
        ...
        if 'District' in self.df.columns:
            ...
            if 'Fire' in district_severity.columns:
                overall_fire_pct = severity_stats['overall_distribution'].get('Fire', {}).get('percentage', 0)
                biased_districts = []
    ...
    def detect_loss_bias(self):
        ...
        loss_by_district = self.df.groupby('District')['total_loss'].agg(
            total='sum',
            mean='mean',
            median='median',
            count='count'
    ...
    def generate_bias_report(self, output_path: str = "data/processed/fire_bias_report.txt"):
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if geo:
                f.write(f"Avg incidents per district: {geo.get('avg_incidents_per_district', 0):.0f}\n\n")
                ...
                for item in geo.get('district_distribution', [])[:5]:
                    f.write(f"  {item['district']}: {item['incident_count']} ({item['percentage']:.2f}%)\n")
            f.write("\n")
            ...
            if temp:
                f.write(f"Expected per time period: {temp.get('expected_percentage', 25):.1f}%\n\n")
                for item in temp.get('time_distribution', []):
                    deviation = item.get('deviation', 0)
                    status = "⚠️ BIASED" if deviation > 10 else "✓ Normal"
                    f.write(f"  {item['time_period']}: {item['incident_count']} ({item['percentage']:.2f}%) [{status}]\n")
            f.write("\n")
            ...
            if loss:
                f.write(f"Overall mean loss per incident: ${loss.get('overall_mean_loss', 0):,.0f}\n\n")
                if loss.get('biased_districts'):
                    f.write("Districts with Disproportionate Losses:\n")
                    for item in loss['biased_districts']:
                        f.write(f"  {item['district']}: ${item['mean_loss']:,.0f} mean loss (deviation: {item['deviation_pct']:.1f}%)\n")
            f.write("\n")
    ...
    def visualize_bias(self, output_dir: str = "data/processed/fire_visualizations"):
        ...
        if 'time_of_day' in self.df.columns:
            ...
            plt.ylabel('Number of Incidents')
            plt.axhline(y=counts.mean(), color='red', linestyle='--', label=f'Average: {counts.mean():.0f}')
            plt.legend()
            plt.tight_layout()
...
def main():
    ...
    print(f"📈 Visualizations: data/processed/fire_visualizations/")
    print("=" * 80 + "\n")
