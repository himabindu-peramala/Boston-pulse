...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/fire_preprocessing.log'),
    ...
    try:
        ...
        if 'Alarm Date' in df.columns and 'Alarm Time' in df.columns:
            try:
                ...
                logger.info("Extracted temporal features: year, month, day_of_week, hour, is_weekend, time_of_day")
...
def generate_statistics(file_path: str = "data/processed/fire_incidents_clean.csv"):
    ...
    try:
        ...
        stats = {
            ...
            'date_range': {
                'start': str(df['alarm_datetime'].min()) if 'alarm_datetime' in df.columns else None,
                'end': str(df['alarm_datetime'].max()) if 'alarm_datetime' in df.columns else None,
            },
            'severity_distribution': df['severity_category'].value_counts().to_dict() if 'severity_category' in df.columns else {},
            'district_distribution': df['District'].value_counts().to_dict() if 'District' in df.columns else {},
            'loss_category_distribution': df['loss_category'].value_counts().to_dict() if 'loss_category' in df.columns else {},
            'time_of_day_distribution': df['time_of_day'].value_counts().to_dict() if 'time_of_day' in df.columns else {},
            'total_property_loss': float(df['Estimated Property Loss'].sum()) if 'Estimated Property Loss' in df.columns else 0,
...
if __name__ == "__main__":
    ...
    print(f"\nDistrict distribution: {dict(list(stats['district_distribution'].items())[:5])}")
    print(f"\nLoss category distribution: {stats['loss_category_distribution']}")
