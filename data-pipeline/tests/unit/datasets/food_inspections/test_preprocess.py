...
from src.datasets.food_inspections.preprocess import (
    FoodInspectionsPreprocessor,
    preprocess_food_inspections_data,
)
...
class TestFoodInspectionsPreprocessor:
    ...
    def test_coordinate_parsing(self, preprocessor, sample_raw_data):
        ...
        # Check value for a specific record
        record_1 = df[df["_id"] == 1].iloc[0]
        assert record_1["lat"] == 42.3456
        assert record_1["long"] == -71.0789
    ...
    def test_preprocess_food_inspections_data_convenience(self, sample_raw_data):
