...
from preprocess_stations import preprocess_stations_data, generate_statistics, haversine_distance
...
def sample_processed_data(tmp_path):
    ...
    data = {
        'NAME': ['DISTRICT A-1 POLICE STATION', 'DISTRICT B-2 POLICE STATION', 'BOSTON POLICE HEADQUARTERS', 'DISTRICT C-6 POLICE STATION'],
        'NEIGHBORHOOD': ['BOSTON', 'ROXBURY', 'ROXBURY', 'SOUTH BOSTON'],
        'DISTRICT': ['A1', 'B2', 'NONE', 'C6'],
...
class TestPreprocessStationsData:
    ...
    def test_output_file_created(self, sample_raw_data, tmp_path):
        ...
        result = preprocess_stations_data(
    ...
    def test_feature_columns_added(self, sample_raw_data, tmp_path):
        ...
        expected_features = ['LAT', 'LON', 'VALID_COORDS', 'DIST_FROM_CENTER_KM',
                             'IS_HEADQUARTERS', 'ZONE', 'FT_SQFT_NORMALIZED']
...
class TestGenerateStatistics:
    ...
    def test_returns_expected_keys(self, sample_processed_data):
        ...
        expected_keys = ['total_stations', 'columns', 'zone_distribution',
                         'size_distribution', 'avg_dist_from_center_km']
