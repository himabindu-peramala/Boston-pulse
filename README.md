...
All dependencies are pinned in `requirements.txt` to ensure consistent environments across machines. The virtual environment isolates project dependencies from system Python.

## Data Schema

### Raw Data Schema (17 columns)

- **INCIDENT_NUMBER**: Unique incident identifier
- **OFFENSE_CODE**: Numeric offense code
- **OFFENSE_CODE_GROUP**: Categorical offense grouping
- **OFFENSE_DESCRIPTION**: Text description of offense
- **DISTRICT**: Police district code (D4, B2, C11, A1, etc.)
- **REPORTING_AREA**: Specific reporting area within district
- **SHOOTING**: Binary indicator for shooting incidents
- **OCCURRED_ON_DATE**: Timestamp of incident occurrence
- **YEAR**: Year extracted from occurrence date
- **MONTH**: Month extracted from occurrence date
- **DAY_OF_WEEK**: Day of week extracted from occurrence date
- **HOUR**: Hour extracted from occurrence date
- **UCR_PART**: Uniform Crime Reporting classification
- **STREET**: Street address of incident
- **Lat**: Latitude coordinate
- **Long**: Longitude coordinate
- **Location**: Combined location string

### Processed Data Schema (26 columns)

Includes all raw columns plus engineered features:
- **year**: Extracted year for temporal analysis
- **month**: Extracted month for seasonal patterns
- **day_of_week**: Day of week (0=Monday, 6=Sunday)
- **hour**: Hour of day for temporal modeling
- **is_weekend**: Binary weekend indicator
- **time_of_day**: Categorical time period (Night, Morning, Afternoon, Evening)
- **valid_coords**: Binary flag for coordinate validity
- **severity**: Crime severity category (Serious, Moderate, Minor)
- **district_crime_count**: Total crimes in that district (for normalization)

## Pipeline Performance

### Execution Metrics

- **Data Ingestion**: Approximately 15-20 seconds for full 2023 dataset
- **Data Validation**: 2-3 seconds for 242,921 records
- **Preprocessing**: 8-10 seconds including feature engineering
- **Bias Detection**: 5-7 seconds including visualization generation
- **Complete Pipeline**: Approximately 30-40 seconds end-to-end

### Data Quality Results

- **Total records**: 242,921
- **Duplicate records**: 0 (0.0%)
- **Missing values**: 561,312 total across all columns
- **Invalid coordinates**: 14,921 records (6.1%)
- **Date range coverage**: January 1, 2023 to January 26, 2026

## Future Enhancements

### Additional Datasets

The pipeline architecture supports expansion to additional Boston Open Data sources:
- 311 Service Requests (civic services)
- Fire Incident Reporting (emergency response)
- MBTA real-time API (public transit)
- Vision Zero Safety Concerns (pedestrian safety)
- Food Establishment Inspections (public health)

Each dataset will follow the same modular pattern: ingestion, preprocessing, validation, versioning.

### Advanced Features

Planned enhancements include:
- Incremental data loading (fetch only new records since last update)
- Continuous training pipeline for ML model retraining
- Advanced anomaly detection using statistical methods
- Real-time data streaming for MBTA and traffic data
- Cross-dataset integration and feature fusion
- Model drift detection and automatic retraining triggers

### Cloud Deployment

Production deployment will migrate to Google Cloud Platform:
- DVC remote storage: Google Cloud Storage buckets
- Airflow deployment: Cloud Composer or Compute Engine
- Database: Cloud SQL for structured data storage
- Monitoring: Cloud Monitoring and Logging

## License

This project uses data from the City of Boston Open Data Portal, released under the Open Data Commons Public Domain Dedication and License (PDDL).

## Contact

For questions or issues, please contact the Boston Pulse team or open an issue in this repository.

## References

- Boston Open Data Portal: https://data.boston.gov
- Crime Incident Reports Dataset: https://data.boston.gov/dataset/crime-incident-reports-august-2015-to-date-source-new-system
